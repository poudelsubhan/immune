...
-> breach_detected. The attack fires via simulate_attack (harness-level,
...
from immune.defender import Defender, simulate_attack
...
def main() -> None:
    ...
    print(f"  fired={trace['fired']}  breach={trace['breach']}  final_response={trace['final_response']!r}")
    assert trace["fired"] is False, f"benign task {task['id']} should not fire a payment"
    ...
    # level, not via the LLM's real judgment — see simulate_attack's
    ...
    trace = simulate_attack(attack, world, guard=None, generation=1)
